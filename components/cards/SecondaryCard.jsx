...
const SecondaryCard = ({ data }) => {
    ...
    return (
        <div className={`${styles.wrapper} flex flex-row-reverse sm:flex-row min-h-full`}>
            <div className={`${styles.image} p-4 basis-1/2 sm:basis-[33%]`}>
            ...
            <div className={`flex flex-col justify-between p-4 basis-1/2 md:basis-3/5 xl:basis-1/2`}>
                <Link href={`${url}`}>
                    <h4 className={`${styles.title} leading-6 text-lg pb-4 md:text-xl text-slate-900 font-bold antialiased tracking-tight xl:text-2xl line-clamp-2`}>{title}</h4>
                </Link>
